...
def convert_pix_lstring_to_geo(params):
    ...
    for i,(x,y) in enumerate(zip(x_pixs, y_pixs)):
        ...
        (lat, lon) = list(tmp_dict.values())[0]
        ...
        with rasterio.open(im_file) as raster_file:
            im_file_crs = raster_file.crs
        df = pd.DataFrame({'x':[lon], 'y':[lat]})
        gdf = geopandas.GeoDataFrame(df, geometry=geopandas.points_from_xy(df.x, df.y), crs=4326).to_crs(im_file_crs)
        x_, y_ = gdf.geometry.x[0], gdf.geometry.y[0]
        
        coords_utm.append([x_, y_])
...
def get_edge_geo_coords(G, im_file, remove_pix_geom=True, fix_utm_zone=True,
    ...
    for i,(u,v,attr_dict) in enumerate(G.edges(data=True)):
        ...
        # Optional : Ensure utm length isn't excessive
...
def wkt_to_G(params):
    ...
    if im_file:
        ...
        with rasterio.open(im_file) as raster_file:
            im_file_crs = raster_file.crs
        G_projected = ox.project_graph(G1, to_crs=im_file_crs)
...
def main():
    ...
    parser.add_argument('--pred_mask_dir', required=True, type=str,
    ...
    assert os.path.exists(args.pred_mask_dir)
    ...
    for image_id in image_ids:
        out_file = os.path.join(args.results_dir, image_id + '_graph.gpickle')
        
        # for geo referencing, im_file should be the raw image
        image_file_name = image_id + '_mask.tif'
        im_file = os.path.join(args.pred_mask_dir, image_file_name)
    ...
    print("Graph gpickle dir: ", args.results_dir)
    t1 = time.time()
