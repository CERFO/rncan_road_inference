...
change datatypes to 64 to accomodate very large files with coords over
  32767'''
...
def neighbors(shape):
    ...
    idx = np.array(idx, dtype=np.uint16).T
...
def build_graph(nodes, edges, multi=False):
    ...
    for i in range(len(nodes)):
        graph.add_node(i, pts=nodes[i], o=np.int64(nodes[i].mean(axis=0)))
...
def buffer(ske):
    buf = np.zeros(tuple(np.array(ske.shape) + 2), dtype=np.uint32)
