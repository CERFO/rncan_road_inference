...
################################################################################
def insert_control_points(G_, control_points, raster_file, max_distance_meters=10,
                          allow_renaming=True,
                          n_nodes_for_kd=1000000, n_neighbors=20,
                          x_coord='x', y_coord='y',
                          verbose=True):
    """
...
def create_graph_midpoints(G_, raster_file, linestring_delta=50, is_curved_eps=0.03,
                           ...
                           verbose=False, super_verbose=False, max_nodes_count=100000):
    """
    ...
    for u, v, data in G_.edges(data=True):
        ...
        if 'geometry' in data:
            ...
            for j, d in enumerate(interp_dists):
                ...
                if len(Gout.nodes()) >= max_nodes_count:
                    raise Exception("Max nodes count reached")
...
def _create_gt_graph(geoJson, im_test_file, network_type='all_private',
    ...
    G0gt_init = nx.read_gpickle(geoJson)
...
################################################################################
def make_graphs(G_gt_, G_p_, im_file,
                weight='length',
                speed_key='inferred_speed_mps',
                travel_time_key='travel_time_s',
    ...
    try:
        G_gt_cp0 = create_graph_midpoints(
            G_gt0.copy(), im_file,
    ...
    try:
        G_p_cp0 = create_graph_midpoints(
            G_p.copy(), im_file,
            linestring_delta=linestring_delta,
            is_curved_eps=is_curved_eps,
            verbose=False,
    ...
    G_p_cp_prime0, xn_p, yn_p = insert_control_points(
        G_p.copy(), control_points_gt, im_file,
...
#         G_gt, control_points_prop, raster_file, max_distance_meters=max_snap_dist,
#         allow_renaming=allow_renaming, verbose=super_verbose)
...
def gather_files(truth_dir, prop_dir,
    ...
    files_too_big = ['AB18_WV02_20140621_graph.gpickle', 'MB10_WV03_20180709_graph.gpickle', 'Moncton1_WV02_20190802_graph.gpickle', 'QC11_WV02_20110619_graph.gpickle']

    name_list = [f for f in os.listdir(truth_dir) if f.endswith('_graph.gpickle') and f not in files_too_big]
    ...
    def get_valid_pairs(i, f):
        ...
        imgid = f.split('.')[0].split('_')[0]
        outroot = gt_file.split('.')[0]
        im_path = gt_file.replace('_graph.gpickle', '_mask.tif')
        # proposal file
        ...
        return G_gt_init, G_p_init, outroot, im_path
    ...
    im_list = filter_none(unzipped[3])

    return gt_list, gp_list, root_list, im_list


###############################################################################
def execute(results_csv_path, gt_list, gp_list, root_list, im_list,
            weight='length',
            speed_key='inferred_speed_mps',
            travel_time_key='travel_time_s',
            max_files=1000,
    ...
    #     file.write('outroot,APLS,APLS_gt_onto_prop,APLS_prop_onto_gt\n')
    ...
    def compute_score_arr(outroot, G_gt_init, G_p_init, im_path, max_nodes_count=999999999):
        # get graphs with midpoints and geometry (if small graph)
        im_file = rasterio.open(im_path)

        C_gt_onto_prop, c, C_prop_onto_gt = 9999, 9999, 9999

        if len(G_gt_init.nodes()) < 100000:  # Huge number, should not be exceeded
            ...
            try:
                G_gt_cp, G_p_cp, G_gt_cp_prime, G_p_cp_prime, \
                    ...
                    all_pairs_lengths_gt_native, all_pairs_lengths_prop_native, \
                    all_pairs_lengths_gt_prime, all_pairs_lengths_prop_prime  \
                    = make_graphs(G_gt_init, G_p_init, im_file,
                                  weight=weight,
                                  speed_key=speed_key,
                                  travel_time_key=travel_time_key,
                                  linestring_delta=linestring_delta,
                ...
                # plot_graph(G_gt_cp), plt.show()  # Debug purpose

                # TODO : Debug function for memory error purpose
                ...
                #         all_pairs_lengths_gt_native, all_pairs_lengths_prop_native, \
                #         all_pairs_lengths_gt_prime, all_pairs_lengths_prop_prime  \
                #         = make_graphs_yuge(G_gt_init, G_p_init, im_file,
                #                            weight=weight,
                #                            speed_key=speed_key,
                #                            travel_time_key=travel_time_key,
                #                            max_nodes=max_nodes,
        ...
        # write results
        with open(results_csv_path, 'a+') as file:
            file.write(outroot + ',' + str(C) + ',' + str(C_gt_onto_prop) + ',' + str(C_prop_onto_gt) + '\n')
    ...
    map_reduce_res = p_umap(compute_score_arr, root_list, gt_list, gp_list, im_list,
                           num_cpus=n_threads)

    # for root_path, gt_path, gp_path, im_path in zip(root_list,gt_list,gp_list,im_list):
    #     compute_score_arr(root_path, gt_path, gp_path, im_path, max_nodes_count)
...
def main():
    ...
    parser.add_argument('--linestring_delta', default=100, type=int,
    ...
    parser.add_argument('--max_nodes', default=100000, type=int,
    ...
    parser.add_argument('--max_nodes_count', default=5000, type=int,
                        help='Maximum number of nodes to inject midpoints.')

    # Not used for project purpose ##############################
    ...
    # Get csv score path
    results_csv_path = args.output_dir + '/' + args.prop_dir.split('/')[-1] + '.csv'
    open(results_csv_path, 'a').close()  # Testing file existence
    ...
    # Gather files
    gt_list, gp_list, root_list, im_list = gather_files(
        args.truth_dir,
        args.prop_dir,
        # im_dir=args.im_dir,
    ...
    # Compute
    execute(
        results_csv_path, gt_list, gp_list, root_list, im_list,
        weight=args.weight,
        speed_key=args.speed_key,
        travel_time_key=args.travel_time_key,
        max_files=args.max_files,
